...
-- 系统字典
INSERT INTO sys_dict (code,
                      name,
                      value_type,
                      status,
                      is_system,
                      remark)
VALUES ('menu_node_type', '菜单节点类型', 'INTEGER', TRUE, TRUE, '系统菜单目录和页面类型'),
...
SELECT dict_info.id,
       ...
       item.is_default,
       item.extra::JSONB
FROM (VALUES ('menu_node_type', '目录', '1', 10, TRUE, '{}'),
...
SELECT organize_info.id,
       ...
       TRUE,
       post_info.name
FROM (VALUES ('product', '产品经理', 10),
             ('project', '项目经理', 20)) AS post_info(code, name, sort_order)
...
SELECT parent_info.id,
       ...
       '系统内置菜单'
FROM (VALUES ('SystemOrganize', '组织管理', '/system/authority/organize', 'system/authority/organize/index', 'organize',
              10, '维护系统组织'),
             ('SystemPost', '岗位管理', '/system/authority/post', 'system/authority/post/index', 'post', 20,
              '维护组织直属岗位'),
             ('SystemUser', '用户管理', '/system/authority/user', 'system/authority/user/index', 'user', 30,
              '维护系统用户'),
             ('SystemMenu', '菜单管理', '/system/authority/menu', 'system/authority/menu/index', 'menu', 40,
              '维护菜单和按钮权限资源'),
             ('SystemRole', '角色管理', '/system/authority/role', 'system/authority/role/index', 'role', 50,
              '维护角色及授权关系')) AS menu_info(name, title, path, component, icon, sort_order, description)
         JOIN sys_menu parent_info
              ON parent_info.name = 'SystemAuthority'
ON CONFLICT DO NOTHING;

-- 当前阶段按钮权限资源
INSERT INTO sys_resource (menu_id,
                          name,
                          code,
                          status,
                          sort_order,
...
SELECT menu_info.id,
       ...
       resource_info.name
FROM (VALUES ('SystemMenu', '新增按钮权限', 'resource:add', 10),
             ...
             ('SystemRole', '配置角色数据权限', 'role:config:data-scope', 60),
             ...
             ('SystemUser', '修改用户状态', 'user:change:status',
              70)) AS resource_info(menu_name, name, code, sort_order)
         JOIN sys_menu menu_info
              ON menu_info.name = resource_info.menu_name
ON CONFLICT (code) DO NOTHING;

-- 内置超级管理员角色
INSERT INTO sys_role (code,
                      name,
                      status,
                      sort_order,
                      remark,
